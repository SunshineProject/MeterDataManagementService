...
DECLARE
	...
	BEGIN
		...
		LOOP
			...
			EXECUTE 'SELECT count(*) FROM procedure WHERE identifier = ''http://www.sunshineproject.eu/swe/procedure/'|| pilot || '-' || trim(both ' ' from to_char(i, '000')) || '''' INTO c;
			raise INFO 'dopo proc %',i;
			IF ( c = 0 )THEN
				INSERT INTO procedure( hibernatediscriminator, proceduredescriptionformatid,identifier, deleted, descriptionfile)
				VALUES (
					'T',
					1, 
					'http://www.sunshineproject.eu/swe/procedure/'|| pilot || '-' || trim(both ' ' from to_char(i, '000')),
					'F',
					null);
			END IF;
		...
		LOOP
			...
			IF (count_name = 0) THEN 
				...
				EXECUTE 'SELECT count(*) FROM featureofinterest WHERE identifier = ''http://www.sunshineproject.eu/swe/featureOfInterest/'|| pilot || '/' || type_of_foi ||'/' ||  trim(both ' ' from to_char(i, '000')) || '''' INTO c;
				...
				IF ( c = 0 )THEN 
					...
					'T',
					6, 
					'http://www.sunshineproject.eu/swe/featureOfInterest/'|| pilot || '/'|| type_of_foi ||'/' ||  trim(both ' ' from to_char(i, '000')),
					codespaceid,
					null,
				...
				EXECUTE 'SELECT featureofinterestid FROM featureofinterest WHERE identifier = ''http://www.sunshineproject.eu/swe/featureOfInterest/'|| pilot || '/'|| type_of_foi ||'/' ||  trim(both ' ' from to_char(i, '000')) || ''' LIMIT 1' into featId;
			...
			SELECT procedure.procedureid INTO procedure_id FROM procedure WHERE identifier LIKE('%' || pilot || '-' || trim(both ' ' from to_char(i, '000')) ||'%') LIMIT 1;
			...
			LOOP
				...
				--se non c'e' gia' inserisco la offering
				EXECUTE 'SELECT count(*) FROM offering WHERE identifier = ''http://www.sunshineproject.eu/swe/offering/' || pilot || '-' || substring(trim(both ' ' from to_char(i, '000')),0,2) || trim(both ' ' from to_char(j, '00')) || '_' || (split_part(list_offering, ';' , j+1):: text) || '''' INTO o;
				
				IF ( o = 0 )THEN --creo la offering solo se non esiste gia'
					INSERT INTO offering( hibernatediscriminator, identifier, name)
					VALUES ('T','http://www.sunshineproject.eu/swe/offering/' || pilot || '-' || substring(trim(both ' ' from to_char(i, '000')),0,2) || trim(both ' ' from to_char(j, '00')) || '_' || (split_part(list_offering, ';' , j+1):: text), null);
				END IF;--o 
				...
				IF ( c > 0 )THEN
					...
					SELECT offeringid INTO offering_id FROM offering WHERE identifier = 'http://www.sunshineproject.eu/swe/offering/' || pilot || '-' || substring(trim(both ' ' from to_char(i, '000')),0,2):: text || trim(both ' ' from to_char(j, '00')) || '_' || (split_part(list_offering, ';' , j+1):: text) LIMIT 1;
